...
IBBadgePopup* IBBadgePopup::create(GJUserScore* score, const std::map<IconType, std::vector<int>>& icons) {
    auto ret = new IBBadgePopup();
    if (ret->initAnchored(300.0f, 70.0f, score, icons)) {
        ret->autorelease();
        return ret;
    }
    delete ret;
...
bool IBBadgePopup::setup(GJUserScore* score, const std::map<IconType, std::vector<int>>& icons) {
    setID("IBBadgePopup");
    ...
    for (auto& [type, ids] : icons) {
        ...
        auto badgeButton = CCMenuItemExt::createSpriteExtraWithFrameName(fmt::format("gj_{}Btn_on_001.png", badgeName), 0.7f, [
            scoreRef = WeakRef(score), type, &ids
        ](auto) {
            if (auto score = scoreRef.lock()) IBIconPopup::create(score, type, ids)->show();
        });
